...
class k3s::config (
  ...
  case $type {
    'init': {
      exec { 'init-cluster':
        command     => "${k3s::binary_path} server --cluster-init --no-deploy traefik",
      ...
      @@exec { 'join-cluster':
        command     => "${k3s::binary_path} server --server ${::ipaddress}:6443 --no-deploy traefik",
        # apparently makes this data dir; https://docs.k3s.io/cli/server#data
        creates     => '/var/lib/rancher/k3s',
        environment => [
          "K3S_TOKEN=${token_secret}"
      ...
      @@exec { 'node-join-cluster':
        command     => "${k3s::binary_path} agent --server ${::ipaddress}:6443 --no-deploy traefik",
        # apparently makes this data dir; https://docs.k3s.io/cli/server#data
        creates     => '/var/lib/rancher/k3s',
        environment => [
          "K3S_TOKEN=${token_secret}"
